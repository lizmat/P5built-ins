...
module P5built-ins:ver<0.0.1> {
    ...
    BEGIN %export{'&' ~ .name} := $_
      for &caller, &chomp, &chop, &chr, &each, &hex, &index, &lcfirst, &length,
          &oct, &ord, &pack, &quotemeta, &ref, &rindex, &substr, &tie, &tied,
          &times, &ucfirst, &unpack, &untie;
}
